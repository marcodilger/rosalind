...
cons.matrix <- matrix( , nrow = 4, ncol = 0)
rownames(cons.matrix) <- c("A","C","T","G")
# as.matrix(mapply(function(x) sum(cur.seq == x), c("A","C","T","G")))
# cons.matrix <- as.matrix(mapply(function(x) sum(cur.seq == x), c("A","C","T","G")))
